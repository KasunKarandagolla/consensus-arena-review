...
const MAX_SUBMIT_ACTION_RETRIES: u32 = 2;
/// Timeout awaiting a fresh `ActiveSubmitReport` ack for each submit attempt.
const SUBMIT_ACK_TIMEOUT_SECS: u64 = 20;
...
// ── Phase 4 Forensic Diagnostics Helpers ───────────────────────────────────────

/// Deterministic non-secret hash for prompt correlation.
/// Uses a simple FNV-1a-like hash for speed and determinism.
...
/// Redact URL for diagnostic logging — keep origin + path, remove query/fragment.
fn redacted_url_for_diag(url: &str) -> String {
    match url.parse::<tauri::Url>() {
        ...
        Err(_) => {
            url.split_once('?').map(|(p, _)| p.to_string()).unwrap_or_else(|| url.to_string())
        }
    }
}
...
// Emits secret-free timing events via arena://log for live latency analysis.
// P7: Every timing record includes agent_id, turn, stage, and elapsed_ms.

fn emit_timing(app: &AppHandle, agent_id: &str, turn: Option<u32>, stage: &str, elapsed_ms: u64) {
    let turn_str = turn.map(|t| t.to_string()).unwrap_or_default();
    let payload = format!("{{\"agent\":\"{}\",\"turn\":\"{}\",\"stage\":\"{}\",\"elapsed_ms\":{}}}", agent_id, turn_str, stage, elapsed_ms);
    let _ = app.emit("pipeline-timing", serde_json::json!({
        "agent_id": agent_id,
        "turn": turn,
        "stage": stage,
...
fn emit_timing_start(app: &AppHandle, agent_id: &str, turn: Option<u32>, stage: &str, start: &Instant) {
    let elapsed = start.elapsed().as_millis() as u64;
    emit_timing(app, agent_id, turn, stage, elapsed);
}

fn emit_timing_elapsed(app: &AppHandle, agent_id: &str, turn: Option<u32>, stage: &str, elapsed_ms: u64) {
    emit_timing(app, agent_id, turn, stage, elapsed_ms);
}
...
async fn confirm_active_submit(
    ...
    for attempt in 0..=MAX_SUBMIT_ACTION_RETRIES {
        ...
        {
            Ok(Ok(Some(response))) => {
                emit_timing_elapsed(app, agent_id, Some(turn), "submit_ack_total", start.elapsed().as_millis() as u64);
                return Ok(SubmitOutcome::ResponseEarly(response));
            }
            Ok(Ok(None)) => {
                // PHASE 4: participant-submit
                let _ = app.emit("participant-submit", serde_json::json!({
                    "agent_id": agent_id,
                    "turn": turn,
                    "prompt_hash": 0u64, // placeholder - prompt not available in this scope
                }));
        ...
        if attempt < MAX_SUBMIT_ACTION_RETRIES {
            emit_timing_elapsed(app, agent_id, Some(turn), "submit_retry", start.elapsed().as_millis() as u64);
...
async fn inject_active_prompt(
    ...
    emit_timing_elapsed(app, agent_id, Some(turn), "active_turn_start", start.elapsed().as_millis() as u64);
    let _ = app.emit("active-turn-state", serde_json::json!({
    ...
    emit_timing_elapsed(app, agent_id, Some(turn), "active_injection_confirmed", inject_start.elapsed().as_millis() as u64);

    let submit_start = Instant::now();
    let outcome = confirm_active_submit(&window, agent_id, turn, nav_rx, app).await?;
    emit_timing_elapsed(app, agent_id, Some(turn), "active_submit_ack", submit_start.elapsed().as_millis() as u64);
    ...
    emit_timing_elapsed(app, agent_id, Some(turn), "active_turn_total", start.elapsed().as_millis() as u64);
    Ok(early_response)
}
...
async fn inject_and_wait_with_retry(
    ...
    for attempt in 0..=MAX_RETRIES {
        ...
        // Extract nav_window and resolve the saved conversation URL.
        // For active turns we REQUIRE the saved conversation URL — never fall back
        // to the base URL, because that creates a fresh chat instead of continuing
        // the existing conversation. If the URL is missing, the route fails
        // immediately with a clear diagnostic so we can fix the root cause.
        tracing::debug!(
            "[LOCK] acquiring browser_state for inject_and_wait_with_retry/{}",
            target_model
        );
        let (nav_window, diagnostics, target_url, saved_url_was_used) = {
            let mut browser = state.browser_state.lock().await;
            let window = crate::browser_backend::ensure_nav_window(app, &mut browser)?;
            let saved_url = browser
                .conversation_urls
                .get(target_model)
                .and_then(|url| url.clone());
            let base_url = crate::browser_backend::get_agent_config(target_model)
                .map(|c| c.base_url.to_string())
                .unwrap_or_else(|| format!("https://chat.{}.com", target_model));
            let target_url = match saved_url {
                Some(url) if url != base_url && url != format!("{}/", base_url) => url,
                _ => {
                    // No saved conversation URL, or saved URL is the base URL (not a real conversation).
                    // This happens when setup completed without a real Send (response_after_injection
                    // or capability_verified). Navigating to the base URL is a no-op — no new Ready
                    // signal fires, and readiness times out. Fail fast with actionable diagnostics.
                    let base_fallback = crate::browser_backend::resolve_participant(target_model, &custom)
                        .map(|info| info.base_url)
                        .unwrap_or_else(|| format!("https://chat.{}.com", target_model));
                    let detail = if saved_url.is_some() {
                        format!(" (saved URL was base URL: {})", base_url)
                    } else {
                        String::new()
                    };
                    return Err(AgentError::NavigationFailed(format!(
                        "No saved conversation URL for {}. Setup must complete with a real send before active turns.{detail} Base fallback would be: {base_fallback}",
                        target_model
                    )));
                }
            };
            (window, browser.diagnostics.clone(), target_url, true)
        }; // lock drops here

        // PHASE 4: Forensic diagnostics — emit participant-route-start with URL hash
        {
            let prompt_hash = hash_prompt(prompt);
            let url_redacted = redacted_url_for_diag(&target_url);
            let leader_id = {
                let browser = state.browser_state.lock().await;
                browser.leader_agent_id.clone()
            };
            let _ = app.emit("participant-route-start", serde_json::json!({
                "agent_id": target_model,
                "turn": turn,
                "source_agent": leader_id,
                "source_turn": iteration,
                "target_url_redacted": url_redacted,
        ...
        // PHASE 4: participant-navigation-ready
        {
            let actual_url = nav_window.url().map(|u| u.to_string()).unwrap_or_else(|_| target_url.clone());
            let url_redacted = redacted_url_for_diag(&actual_url);
            let _ = app.emit("participant-navigation-ready", serde_json::json!({
                "agent_id": target_model,
                "turn": turn,
                "actual_url_redacted": url_redacted,
            }));
        }
        ...
        // PHASE 4: participant-injection-start
        {
            let prompt_hash = hash_prompt(prompt);
            let _ = app.emit("participant-injection-start", serde_json::json!({
                "agent_id": target_model,
                "turn": turn,
        ...
        .await
        {
            ...
            Ok(()) => {
                // PHASE 4: participant-injection-confirmed
                {
                    let prompt_hash = hash_prompt(prompt);
                    let _ = app.emit("participant-injection-confirmed", serde_json::json!({
                        "agent_id": target_model,
                        "turn": turn,
                        "prompt_length": prompt.len(),
                        "prompt_hash": prompt_hash,
                        "composer_tag": "unknown", // filled by browser if needed
                        "composer_role": "unknown",
        ...
        match wait_for_response(target_model, turn, nav_rx).await {
            Ok(response) => {
                ...
                let response_hash = hash_prompt(&response);
                let _ = app.emit("participant-response", serde_json::json!({
                    "agent_id": target_model,
                    "turn": turn,
