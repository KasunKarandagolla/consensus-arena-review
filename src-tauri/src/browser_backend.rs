...
impl BrowserDiagnostics {
    ...
    /// Clear the last_ready_at timestamp for an agent to force a fresh ready wait.
    /// Used before participant navigation to avoid consuming a stale setup-phase Ready signal.
    pub fn clear_ready_timestamp(&self, agent_id: &str) {
...
mod tests {
    ...
    // ── P5: Send candidate scoring regression ───────────────────────────────────
    // Ensures an attachment button appearing before the real Send button in DOM order
    // is never selected, because semantic scoring outranks DOM order.

    #[test]
    fn generic_init_send_scoring_rejects_attachment_button() {
        // The attachment button has aria-label="Attach files" and data-testid containing "attach"
        // The real Send button has aria-label="Send message" and data-testid containing "send"
        // Both are in the same composer root. Scoring must prefer the Send button.
        for required in [
            "scoreIconOnlySend",
            "bestStrongScore",
            "bestIconScore",
            "PASS 1: Strong send candidates only",
            "PASS 2: Icon-only fallback",
            "isAttachmentLike",
            "isFileUploadLike",
            "isPaperclip",
            "isVoice",
            ...
            assert!(
                GENERIC_INIT_SCRIPT.contains(required),
                "send scoring marker missing: {required}"
            );
        }
        // The negative filter must reject attachment buttons even if they have SVG icons
        // P7: Check actual attribute values (testid, class, aria-label, title) not just text
        assert!(
            GENERIC_INIT_SCRIPT.contains("testId.indexOf('attach')") &&
            GENERIC_INIT_SCRIPT.contains("classAttr.indexOf('attach')") &&
            GENERIC_INIT_SCRIPT.contains("ariaLabel.indexOf('attach')") &&
            GENERIC_INIT_SCRIPT.contains("title.indexOf('attach')"),
            "attachment negative filter must check testid/class/aria-label/title for attach"
        );
        // Strong candidates get 1000 base score, icon-only max ~90
        assert!(
            GENERIC_INIT_SCRIPT.contains("var score = 1000") && GENERIC_INIT_SCRIPT.contains("item.iconScore"),
            "strong send candidates must score much higher than icon-only"
        );
    }

    #[test]
    fn generic_init_icon_only_scoring_is_low_priority() {
        // Icon-only candidates (scoreIconOnlySend) max at ~90 (40+30+20), while strong
        // send candidates get 1000 base + 100 tie-breaker. This ensures semantic send
        // buttons always win over icon-only fallbacks.
        assert!(
            GENERIC_INIT_SCRIPT.contains("score += 40") &&
            GENERIC_INIT_SCRIPT.contains("score += 30") &&
            GENERIC_INIT_SCRIPT.contains("score += 20"),
            "icon-only scoring components present"
        );
        assert!(
            GENERIC_INIT_SCRIPT.contains("item.iconScore"),
            "iconScore used in findEnabledButton"
        );
    }
}
...
(function() {
    ...
    function isSendCandidate(el) {
        ...
        // Strong positive: explicit send/submit/arrow indicators
        if (text.indexOf('send') !== -1 || text.indexOf('submit') !== -1 ||
            text.indexOf('arrow-up') !== -1 || text.indexOf('arrow up') !== -1) return true;
        // Selector-based positive match
        return el.matches && SEND_SELECTORS.some(function(selector) {
            try { return el.matches(selector); } catch (e) { return false; }
        });
    }

    // Returns a confidence score (0-100) for icon-only send buttons.
    // Low priority fallback: only used when no strong send candidate exists.
    // P7: Forbidden controls (attachment/upload/paperclip/voice/stop) are EXCLUDED entirely.
    function scoreIconOnlySend(button, input) {
        if (!button || !(button instanceof Element) || !isVisible(button) || !isEnabled(button)) return 0;
        ...
        if (button.querySelector('path[d]')) score += 30;
        // Proximity bonus
        if (inputRect) {
            var vDist = Math.min(Math.abs(rect.top - inputRect.top), Math.abs(rect.bottom - inputRect.bottom));
            if (vDist < 20) score += 20;
            else if (vDist < 50) score += 10;
        }
        return score;
    }
    ...
    // (scoreIconOnlySend) is only a secondary check on candidates already owned
    ...
    function collectSendCandidatesIn(root, input) {
        ...
        function consider(button) {
            ...
            if (!isVisible(button) || !isEnabled(button)) return;
            if (candidates.some(function(c) { return c.el === button; })) return;
            var isStrong = isSendCandidate(button);
            var iconScore = isStrong ? 0 : scoreIconOnlySend(button, input);
            if (isStrong || iconScore > 0) {
                candidates.push({ el: button, strong: isStrong, iconScore: iconScore });
            }
        }
    ...
    function emitSendProbe(force) {
        ...
        const send = sendCandidates.length > 0 ? sendCandidates[0].el : null;
    ...
    window.__caSubmitActivePrompt = function(input, expectedAgentId, expectedTurn) {
        ...
        function findEnabledButton() {
            ...
            // P7: Comprehensive diagnostic tracing for send candidate resolution.
            // Emits detailed candidate info via arena://log for live debugging.
            var agentId = getAgentId();
            function logSendDiag(phase, data) {
                try {
                    var payload = JSON.stringify({ phase: phase, agent: agentId, data: data });
                    window.location.href = 'arena://log/debug/send-diag/' + agentId + '/' + encodeURIComponent(payload);
                } catch (e) {}
            }
            function describeCandidate(button) {
                if (!button) return null;
                var rect = button.getBoundingClientRect();
                var inputRect = found.input && found.input.getBoundingClientRect ? found.input.getBoundingClientRect() : null;
                var vDist = inputRect ? Math.min(Math.abs(rect.top - inputRect.top), Math.abs(rect.bottom - inputRect.bottom)) : null;
                return {
                    tag: button.tagName,
                    ...
                    hasPath: !!button.querySelector('path[d]'),
            ...
            logSendDiag('findEnabledButton-start', { compositeCount: composite.length });

            // PASS 1: Strong send candidates only (explicit send/submit/arrow or SEND_SELECTORS match)
            var bestStrong = null;
            var bestStrongScore = -1;
            for (var i = 0; i < composite.length; i++) {
                var item = composite[i];
                var candidate = item.el;
                if (!candidate || candidate.disabled || candidate.getAttribute('aria-disabled') === 'true') continue;
                if (!item.strong) continue; // Only strong candidates in PASS 1
                var score = 1000; // base for strong
                var text = candidateText(candidate);
                if (text.indexOf('send') !== -1 || text.indexOf('submit') !== -1 || text.indexOf('arrow') !== -1) {
                    score += 100;
                }
                logSendDiag('candidate-strong', { index: i, score: score, desc: describeCandidate(candidate) });
            ...
            if (bestStrong) {
                logSendDiag('findEnabledButton-result', { pass: 1, selected: describeCandidate(bestStrong), score: bestStrongScore });
                return bestStrong;
            }
            // PASS 2: Icon-only fallback (only if no enabled strong candidate exists)
            // Forbidden controls (attachment/upload/paperclip/voice/stop/cancel) are EXCLUDED
            var bestIcon = null;
            var bestIconScore = -1;
            for (var i = 0; i < composite.length; i++) {
                var item = composite[i];
                ...
                var score = item.iconScore;
                logSendDiag('candidate-icon', { index: i, score: score, desc: describeCandidate(candidate) });
                if (score > bestIconScore) {
                    bestIconScore = score;
                    bestIcon = candidate;
                }
            }
            logSendDiag('findEnabledButton-result', { pass: 2, selected: bestIcon ? describeCandidate(bestIcon) : null, score: bestIconScore });
            return bestIcon;
        }
