use crate::browser_backend::{
    NavEvent, READINESS_WAIT_TIMEOUT_SECS, display_name_for, navigate_agent_window,
    record_browser_blocker, record_browser_error,
...
pub async fn run_setup(
    ...
    for agent_id in &setup_order {
        ...
        // (SendDetected or UserConfirmedManual). For ResponseAfterInjection and
        // capability_verified, no conversation was created — the URL would be
        // the base URL, which is useless for active routing. We explicitly store
        // None so active routing fails fast with a clear error instead of
        // silently navigating to the base URL.
        if setup_completed_with_real_send {
            let url = window.url().map(|url| url.to_string()).map_err(|error| {
                let message = format!("failed to read current conversation URL: {error}");
                record_browser_error(app, &diagnostics, agent_id, &message);
                AgentError::NavigationFailed(message)
            })?;
            conversation_url = Some(url);
        }

        // Save URL to vault only if we have a real conversation URL.
        if let Some(ref url) = conversation_url {
        ...
        // Save URL to browser state (None if no real conversation was created)
        {
            let mut browser = state.browser_state.lock().await;
            browser
                .conversation_urls
                .insert(agent_id.clone(), conversation_url.clone());
        }

        let emit_url = conversation_url.clone().unwrap_or_default();
        app.emit(
            "setup-agent-complete",
            json!({
                "agent_id": agent_id,
                "conversation_url": emit_url
            }),
        )
        .ok();
    }
