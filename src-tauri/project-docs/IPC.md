...
// Also includes leader_window_exists, nav_window_exists (both retained for
// compatibility and defined as Tauri lookup results),
// leader_window_handle_exists, nav_window_handle_exists,
// leader_tauri_lookup_exists, nav_tauri_lookup_exists, and
...
// readiness_timeout_ms, readiness_probe_count, readiness_stable_probe_count,
// readiness_elapsed_ms, input_candidate_count, composer_candidate_count,
// send_button_candidate_count, page_state_hint, page_health_hint,
// automation_present, automation_version, capability_input_found,
// capability_send_found, capability_reason, hydrated_but_send_missing,
// active_expected_agent_id, active_turn_number, active_target_window_label,
// active_window_identity_agent_id, active_window_identity_ok,
// active_automation_present, active_automation_version,
// last_active_prompt_injected_at,
...
// active_submit_at. Readiness is capability-based: automation, matching
// identity, and the current composer are required, but account/profile/header
// controls are not. These diagnostics never include prompt or response text.
// current_phase is queued | creating |
// loading | ready | waiting_user_send | consulting |
// active_turn_started | active_prompt_injected | active_prompt_submitted |
// active_waiting_for_response | active_manual_recovery |
// active_response_captured | active_turn_failed | active_window_lost |
...
listen('setup-agent-complete', (event) => {
    ...
    // Strong browser send proof or explicit user confirmation primed this one agent.
    // SetupResponseObserved, Response, and Done are weak diagnostics only and
    // never complete setup or enter active debate content.
...
listen('setup-complete', (event) => {
    // All selected agents produced strong setup proof (same-agent send
    // detection or explicit manual confirmation). The active loop is starting.
    // No payload
})

listen('active-turn-state', (event) => {
    const { event, agent_id, turn_number, error } = event.payload
    // event: 'active_turn_started' | 'active_prompt_injected' |
    //        'active_prompt_submitted' | 'active_submit_failed' |
    //        'active_manual_recovery' | 'active_window_closed' |
    //        'active_waiting_for_response' | 'active_response_captured' |
    //        'active_turn_timeout'
    // Safe progress only: contains no prompt or response text.
    // error is optional, secret-free submit failure metadata such as
    // 'prompt_integrity_failed' or 'active_submit_report_missing'.
    // active_manual_recovery keeps this exact agent/turn eligible for browser
    // capture or provide_manual_model_response.
})
```
