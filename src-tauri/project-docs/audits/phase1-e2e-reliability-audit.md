...
- Batch A.1 follow-up: chunk signals are emitted through a paced 75ms queue,
  router assembly accepts start/chunk/end in any order, and `Done` no longer
  sets `last_active_response_at`.
...
## Batch A.2 — participant active-route readiness and lifecycle truth

- Active participant routing now reuses the shared nav WebView when it is
  already assigned to the target, diagnostics prove `input_found` plus a
  `composer_detected` page hint, and either a real navigation URL or saved
  conversation URL exists. Reuse injects directly without waiting for a fresh
  SPA `arena://ready` signal; setup readiness remains unchanged.
- A fresh-navigation ready timeout receives one same-window direct-injection
  retry only when those composer diagnostics become true.
- Active diagnostics now distinguish `active_turn_started`, injected, waiting,
  captured, and failed. `clear_active_turn` only clears ownership; it no longer
  records a fake response timestamp or captured phase.

## Batch A.3 — recoverable multi-model active auto-submit

- Active auto-submit now tries semantic send controls, composer-scoped and
  document-scoped rightmost safe clickables, form submission, then a focused
  Enter fallback. It excludes attachment, voice, reasoning, search, and Stop
  controls; diagnostics retain only method/status metadata.
- Prompt-injection and active-submit reports are delayed so their arena
  navigation signals cannot overwrite one another in the same JS stack.
- An active auto-submit failure is recoverable: the exact turn stays waiting
  for a manual click, browser capture, or the existing manual pasted response.

## Batch A.4 — acknowledged critical active-turn transport

- Active submit and response start/chunk/end/done signals are serialized by a
  generic in-page queue and retried every 250ms until Rust evaluates the
  matching acknowledgement, with a bounded retry count.
- The arena parser accepts both legacy URLs and signal-ID-prefixed URLs;
  response chunk assembly remains idempotent under delivery retries.
- Active prompt-injection reports use the same ACK queue during auto-submit,
  while setup remains legacy-compatible. Closed active WebViews emit a direct
  `active_window_closed` recovery status instead of waiting for response timeout.
- Retransmitted response start/chunk/end signals are duplicate-safe: starts and
  ends are idempotent and chunk indexes overwrite identical retries. No prompt
  or response text is added to browser diagnostics.

## Batch A.5 — composer acceptance and safe submit scope

- Post-A.4 failures were composer acceptance/integrity failures, not transport.
  Active injection now uses textarea and contenteditable/ProseMirror-compatible
  adapter paths with normalized, secret-free integrity checks.
- The unsafe document-wide rightmost-click fallback is removed; send controls
  remain composer-scoped, with a rejection marker for unsafe candidates.
- Active turns now report an explicit `prompt_integrity_failed` or
  `active_submit_report_missing` failure while retaining the exact turn for a
  manual Send or pasted response; diagnostics remain secret-free.
- Participant routing now fails immediately with `active_window_closed` when
  the shared nav WebView is missing or closed, rather than recreating a window
  or waiting for the 300-second response timeout.
- Browser-script tests assert adapter and safe-click markers, including the
  absence of `document_rightmost_pointer_click`. DOM-level adapter behavior is
  provider-runtime dependent, so the focused tests validate generated-script
  contracts rather than simulating provider frameworks.
