...
export default function SetupView(){
  ...
  const deliveryBrainReady=Boolean(brain.api_key.trim()&&brain.base_url.trim()&&brain.model.trim())
  const canStart=mode==='delivery' ? Boolean(setupBrief.trim()&&projectPath.trim()&&dshPrerequisite?.compatible&&deliveryBrainReady&&!loading) : Boolean(setupBrief.trim()&&selected.size>=2&&selected.has(leader)&&brainReady&&!loading)
  useEffect(()=>{if(mode==='delivery'&&!deliveryBrainReady)setOpen(true)},[mode,deliveryBrainReady])
  ...
  async function start(){
    if(!canStart){const missingDeliveryBrain=mode==='delivery'&&!deliveryBrainReady;setError(missingDeliveryBrain?'Configure the primary Agent Brain API key, base URL, and model before Build mode.':mode==='delivery'?'Choose a project folder and describe the desired outcome.':'Complete the brief and agent brain configuration, then select at least 2 participants.');if(mode==='consult'||missingDeliveryBrain)setOpen(true);return}
    ...
    }catch(e){const message=buildCommandErrorMessage(commandName,e);console.error(message);setError(message);addToast(message,7000)}finally{setLoading(false)}
  }
  ...
  return <section className="view"><Topbar title="New session"/><div className="scroll pt" style={{display:'flex',flexDirection:'column',alignItems:'center'}}><div className="fw">
    ...
    {mode==='consult'&&<div className="fg"><label className="fl">Leader model</label><select className="fi" disabled={loading} value={leader} onChange={e=>setLeader(e.target.value)}>{participants.filter(p=>selected.has(p.agent_id)).map(p=><option value={p.agent_id} key={p.agent_id}>{p.display_name}</option>)}</select></div>}
     {mode==='delivery'&&!deliveryBrainReady&&<div className="brain-note" role="status"><Network size={15}/><span>Build mode needs the primary Agent Brain API key, base URL, and model. Configure them below to continue.</span></div>}
     <div className="fg"><div className="coll-h" onClick={()=>setOpen(v=>!v)} style={{borderRadius:open?'10px 10px 0 0':10}}><span><Cpu size={15}/>Agent brain</span>{open?<ChevronUp size={14}/>:<ChevronDown size={14}/>}</div>{open&&<div className="coll-body">
      <input className="fi" placeholder="API base URL" disabled={loading} value={brain.base_url} onChange={e=>setBrain({...brain,base_url:e.target.value})}/><input className="fi" type="password" placeholder="API key" disabled={loading} value={brain.api_key} onChange={e=>setBrain({...brain,api_key:e.target.value})}/><input className="fi" placeholder="Model name" disabled={loading} value={brain.model} onChange={e=>setBrain({...brain,model:e.target.value})}/><textarea className="fi" style={{minHeight:72}} placeholder="System prompt..." disabled={loading} value={brain.system_prompt} onChange={e=>setBrain({...brain,system_prompt:e.target.value})}/>
