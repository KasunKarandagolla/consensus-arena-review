...
export default function SetupView(){
  ...
  return <section className="view"><Topbar title="New session"/><div className="scroll pt" style={{display:'flex',flexDirection:'column',alignItems:'center'}}><div className="fw">
    ...
    {mode==='consult'&&<div className="fg"><label className="fl">Leader model</label><select className="fi" value={leader} onChange={e=>setLeader(e.target.value)}>{participants.filter(p=>selected.has(p.agent_id)).map(p=><option value={p.agent_id} key={p.agent_id}>{p.display_name}</option>)}</select></div>}
