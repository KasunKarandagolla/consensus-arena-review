...
export default function AskUserPopup(){
  ...
  const answer=useCallback(async(value:string)=>{if(answering.current)return;answering.current=true;try{await invoke('provide_user_answer',{answer:value});setAskUserPending(null)}catch(e){console.error('provide_user_answer failed',e);answering.current=false}},[setAskUserPending])
  ...
  useEffect(()=>{if(!askUserPending?.allow_custom)return;const timer=window.setTimeout(()=>input.current?.focus(),80);return()=>window.clearTimeout(timer)},[askUserPending])
