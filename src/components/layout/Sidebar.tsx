...
import { AGENT_IDS, displayName } from '@/lib/agents'
...
export default function Sidebar(){
  const {selectedSessionId,recoveryState,settingsOpen,sidebarCollapsed,sessionStatus,setSessionStatus,setSelectedSessionId,setRecoveryState,clearSessionState,setSettingsOpen,addToast}=useAppStore()
  ...
  async function details(id:string){setMenu(null);try{const data=JSON.parse(await invoke<string>('get_session_details',{session_id:id})) as Details;addToast(`${data.status} · ${data.turn_count} turns · ${data.section_count} sections · ${data.agent_ids.map(displayName).join(', ')||'No agents yet'}`,5000)}catch(e){console.error(e);addToast('Could not load details')}}
  ...
  return <><aside className={`sidebar${sidebarCollapsed?' closed':''}`}><div className="sb-inner"><div className="sb-top"><div className="sb-logo-row"><span className="sb-logo">Consensus&nbsp;<em style={{fontStyle:'normal'}}>Arena</em></span></div><button className="sb-new" onClick={newSession}><SquarePen size={15}/>New session</button>{recoveryState&&<div className="recover-card"><RotateCcw size={15}/><span>Recover incomplete session</span><button onClick={()=>void recover()}>Recover</button></div>}</div>
    ...
    <div className="sb-foot"><div className="sb-models"><span className="sb-mlbl">Connected models</span><div className="mdots">{AGENT_IDS.map(id=><span className={`md${health[id]?.is_available?' on':' off'}`} title={`${displayName(id)}${health[id]?.last_error?`: ${health[id].last_error}`:''}`} key={id}/>)}</div></div><div className="acct" onClick={()=>setSettingsOpen(true)}><div className="ava">CA</div><span className="acct-n">Account</span><Settings2 size={15}/></div></div>
  </div></aside>
