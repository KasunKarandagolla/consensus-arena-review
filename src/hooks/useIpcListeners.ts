  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      cleanups.push(await listen('active-turn-state', (e) => {
        const { event, agent_id, turn_number, error } = e.payload as {
          event: string; agent_id: string; turn_number: number; error?: string
        }
        ...
        if (event === 'active_submit_failed') {
          ...
          const recoverable = new Set([
            'prompt_integrity_failed', 'integrity_failed', 'active_submit_report_missing',
            'input_not_found', 'composer_not_found', 'submit_helper_missing',
            'enabled_send_button_not_found_after_retry', 'active_submit_failed',
          ])
          store.setLiveStatusText(recoverable.has(error || '')
            ? 'Manual recovery available: click Send in the model window; Arena will continue automatically, or paste the visible response.'
            : 'Prompt was inserted but not auto-sent. Click Send in the model window, then Arena will continue automatically, or paste the visible response.')
        }
        if (event === 'active_manual_recovery') {
          store.setActiveTurn(agent_id, turn_number)
          store.setLiveStatusText('Manual recovery available: click Send in the model window; Arena will continue automatically, or paste the visible response.')
        }
        ...
        if (event === 'active_window_closed') {
          store.setActiveTurn(agent_id, turn_number)
          store.setLiveStatusText('The model window was closed. Reopen/retry the session or restore the model window.')
        }
      }))
