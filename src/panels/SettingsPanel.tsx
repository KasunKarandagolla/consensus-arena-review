    ...
    <aside className="sp">
      ...
      <div className="sp-body">
        <Section icon={<Wifi size={12} />} title="Connected accounts">
          {participants.map((p) => { const on = health[p.agent_id]?.is_available; return <div className="cr" key={p.agent_id}><span className={`cdot ${on ? 'on' : 'off'}`} /><span className="cr-n">{p.display_name}</span><button className="cr-btn" disabled title={p.is_custom ? 'Custom AI — log in manually in its window' : 'Account login is managed in each model WebView'}>{on ? 'Available' : 'Not checked'}</button></div> })}
        </Section>
